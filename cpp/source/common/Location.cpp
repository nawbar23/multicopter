...
{
	p = getInvalidPosition();
	absoluteAltitude = getInvalidAltitude();
	relativeAltitude = getInvalidAltitude();
}

Location::Location(const Vect2Dd& _p, const float _absoluteAltitude, const float _relativeAltitude):
p(_p), absoluteAltitude(_absoluteAltitude), relativeAltitude(_relativeAltitude)
{
}

bool Location::isValidPosition(void) const
{
	return p != getInvalidPosition();
}
