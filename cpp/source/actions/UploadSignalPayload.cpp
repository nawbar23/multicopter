...
void UploadSignalPayload::handleReception(const IMessage& message)
{
    ...
    default:
        except("Message received in unexpected state");
    }
}
...
void UploadSignalPayload::handleSignalReception(const Parameter parameter)
{
    ...
    case UPLOAD:
        ...
        case SignalData::ACK:
            ...
            monitor->notifyUavEvent(new UavEventUploaded(data));
            state = IDLE;
        ...
        case SignalData::TIMEOUT:
            ...
            {
                except("Retransmission counter exceeded when uploading signal paylod data");
            }
            break;
...
{
    // TODO refactor protocol to CONTROL_SETTINGS(type):DOWNLOAD/UPLOAD signaling
    switch (type)
    {
